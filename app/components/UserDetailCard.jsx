...
const UserCard = ({ user, onPromote }) => {
  ...
  const userId = user.id || user.email || `${user.name}-${user.department}`.replace(/\s+/g, '-').toLowerCase();
  const userWithId = { ...user, id: userId };

  const handleBookmark = () => {
    if (isBookmarked(userWithId)) {
      removeBookmark(userId);
    } else {
      addBookmark(userWithId);
    }
  };
  ...
  const confirmPromote = () => {
    if (onPromote && userId && selectedPosition) {
      onPromote(userId, selectedPosition);
        ...
        <div className="flex justify-between space-x-2 mb-2">
          <Link
            href={{
              ...
              query: { user: JSON.stringify(userWithId) },
            }}
          >
          ...
          <button
            ...
            className={`${
              isBookmarked(userWithId) ? "bg-red-500" : "bg-yellow-500"
            } text-white px-3 py-1 rounded-full text-sm`}
          >
            {isBookmarked(userWithId) ? "Unbookmark" : "Bookmark"}
          </button>
